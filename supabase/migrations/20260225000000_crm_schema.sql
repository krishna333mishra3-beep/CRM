-- ========================================================
-- PRODUCTION MULTI-TENANT CRM SCHEMA & RLS POLICIES
-- ========================================================
...
-- Roles: Owner, Admin, Manager, Sales Executive, Employee, Viewer
...
CREATE TABLE IF NOT EXISTS public.pipeline_stages (
    ...
    name VARCHAR(255) NOT NULL,
    code VARCHAR(50) NOT NULL, -- NEW, CONTACTED, QUALIFIED, PROPOSAL, NEGOTIATION, WON, LOST
...
-- Sources: WEBSITE, INSTAGRAM, FACEBOOK, GOOGLE, WHATSAPP, REFERRAL, COLD_CALL, ADVERTISEMENT, SCRAPED, MANUAL, OTHER
...
-- 9. ACTIVITIES (Calls, Emails, WhatsApp, Meetings, Notes, Follow-ups, Status Changes)
CREATE TABLE IF NOT EXISTS public.activities (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    organization_id UUID NOT NULL REFERENCES public.organizations(id) ON DELETE CASCADE,
    type VARCHAR(50) NOT NULL, -- CALL, EMAIL, WHATSAPP, MEETING, NOTE, FOLLOW_UP, STATUS_CHANGE, STAGE_CHANGE, ASSIGNMENT
...
CREATE TABLE IF NOT EXISTS public.notifications (
    ...
    type VARCHAR(50) DEFAULT 'INFO', -- INFO, LEAD_ASSIGNED, DEAL_ASSIGNED, TASK_DUE, STAGE_CHANGE
    link TEXT,
...
CREATE TABLE IF NOT EXISTS public.audit_logs (
    ...
    organization_id UUID NOT NULL REFERENCES public.organizations(id) ON DELETE CASCADE,
    user_id UUID REFERENCES public.profiles(id) ON DELETE SET NULL,
    action VARCHAR(100) NOT NULL, -- LEAD_CREATED, LEAD_UPDATED, LEAD_DELETED, DEAL_STAGE_CHANGED, etc.
    entity_type VARCHAR(50) NOT NULL, -- LEAD, CONTACT, COMPANY, DEAL, TASK, SETTING, USER
    entity_id UUID,
    metadata JSONB DEFAULT '{}'::jsonb,
    created_at TIMESTAMPTZ DEFAULT timezone('utc'::text, now()) NOT NULL
);
...
-- 15. ATTACHMENTS (Supabase Storage reference)
...
-- Profiles: user can read any profile within same organization, update own profile
...
-- Generic macro RLS for organization owned tables:
-- Leads
